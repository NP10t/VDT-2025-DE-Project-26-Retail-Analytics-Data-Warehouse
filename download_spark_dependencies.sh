...
for url in "${urls[@]}"; do
  ...
  if [ -f "volumes/spark_jars/$filename" ]; then
    echo "Skipping $filename (already exists)"
    continue
  fi
  echo "Downloading $url"
  wget -q --show-progress -P volumes/spark_jars "$url"
done

echo "✅ All JARs downloaded to ./volumes/spark_jars"